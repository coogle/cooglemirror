...
class CronRunCommand extends Command {
	...
	public function everyFiveMinutes(callable $callback)
	{
		if((int) date('i', $this->timestamp) % 5 === 0) call_user_func($callback);
	}

	public function everyTenMinutes(callable $callback)
	{
		if((int) date('i', $this->timestamp) % 10 === 0) call_user_func($callback);
	}

	public function everyFifteenMinutes(callable $callback)
	{
		if((int) date('i', $this->timestamp) % 15 === 0) call_user_func($callback);
	}

	public function everyThirtyMinutes(callable $callback)
	{
	 ...
	 */
	public function hourly(callable $callback)
	{
	 ...
	 */
	public function hourlyAt($minute, callable $callback)
	{
	 ...
	 * Called every day
	 */
	public function daily(callable $callback)
	{
	 ...
	 */
	public function dailyAt($time, callable $callback)
	{
	 ...
	 */
	public function twiceDaily(callable $callback)
	{
	 ...
	 */
	public function weekly(callable $callback)
	{
		if(date('D', $this->timestamp) === 'Sun' && date('H:i', $this->timestamp) === $this->runAt) call_user_func($callback);
	}
	 ...
	 * @param  string $time [HH:MM]
	 */
	public function weeklyOn($day, $time, callable $callback)
	{
	 ...
	 */
	public function monthly(callable $callback)
	{
	 ...
	 */
	public function yearly(callable $callback)
	{
